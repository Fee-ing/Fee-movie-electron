const WEBCONFIG = {
  'common': {
    'one': {
      'name': '80s电影网',
      'favicon': 'http://www.80s.tw/favicon.ico',
      'desc': '高清手机电影迅雷下载_最新MP4电视剧磁力下载',
      'type': '1',
      'home': 'https://www.80s.tt/',
      'home2': 'http://www.80s.tw/',
      'href': 'https://www.80s.tt',
      'href2': 'http://www.80s.tw',
      'path': '/one',
      'show': true
    },
    'two': {
      'name': 'LOL电影天堂',
      'favicon': 'http://www.loldytit.com/favicon.ico',
      'desc': '最新电影迅雷下载_电影天堂_全集网_高清首发',
      'type': '2',
      'home': 'http://www.loldytit.com/',
      'href': 'http://www.loldytit.com',
      'path': '/two',
      'show': true
    },
    'three': {
      'name': '电影天堂',
      'favicon': 'http://www.dytt8.net/favicon.ico',
      'desc': '免费电影_迅雷电影下载',
      'type': '3',
      'home': 'http://www.dytt8.net/index.htm',
      'href': 'http://www.dytt8.net',
      'path': '/three',
      'show': true
    },
    'four': {
      'name': '高清电台',
      'favicon': 'http://gaoqing.fm/favicon.ico',
      'desc': '高清电影、电影字幕、在线点播、磁力链接、BT种子下载聚合网站！',
      'type': '4',
      'home': 'https://gaoqing.fm/',
      'search': 'https://gaoqing.fm/s.php',
      'path': '/four',
      'show': false
    },
    'five': {
      'name': '电影港',
      'favicon': 'http://www.dygang.net/favicon.ico',
      'desc': '高清电影下载_720p高清_1080p高清',
      'type': '5',
      'home': 'http://www.dygang.net/',
      'path': '/five',
      'show': false
    },
    'six': {
      'name': '去转盘网',
      'favicon': 'http://www.quzhuanpan.com/media/image/favicon.ico',
      'desc': '影视列表|网盘搜索引擎|网盘资源搜索下载网站！',
      'type': '6',
      'home': 'http://www.quzhuanpan.com/source/loadSourceList.do?type=1',
      'href': 'http://www.quzhuanpan.com',
      'path': '/six',
      'show': false
    },
    'seven': {
      'name': '人人影视',
      'favicon': 'http://www.zimuzu.tv/favicon.ico',
      'desc': '字幕组下载站|为您翻译最新最快的海外影视剧,ZiMuZu.tv',
      'type': '7',
      'home': 'http://www.zimuzu.tv/',
      'href': 'http://www.zimuzu.tv',
      'path': '/seven',
      'show': true
    },
    'eight': {
      'name': '韩饭网',
      'favicon': 'http://www.hanfan.cc/favicon.ico',
      'desc': '韩国娱乐新闻、综艺、KPOP、韩剧',
      'type': '8',
      'home': 'http://www.hanfan.cc/',
      'path': '/eight',
      'show': true
    }
  },
  'search': {
    'one': {
      'search': 'https://www.80s.tt/search',
      'href': 'https://www.80s.tt'
    },
    'six': {
      'search': 'http://www.quzhuanpan.com/source/search.action',
      'href': 'http://www.quzhuanpan.com'
    },
    'eight': {
      'search': 'http://www.hanfan.cc/'
    }
  }
}

module.exports = WEBCONFIG
